...
export const fetchWorkspaces = async (apiKey?: string): Promise<any[]> => {
  try {
    ...
    // Simple fetch without retries - if it fails, it fails fast
    ...
    if (response.status === 429) {
      throw new Error(`Motion API rate limit exceeded (429). Please wait a few minutes before trying again.`);
    }
